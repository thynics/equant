...
def _required_model_files(model_dir: Path) -> list[str]:
    ...
    return base_files + shard_files + ["model.safetensors.index.json"]


def model_snapshot_complete(model_dir: Path) -> bool:
    if not model_dir.exists():
        return False
    for filename in _required_model_files(model_dir):
        if not (model_dir / filename).exists():
            return False
    return True


def vendor_snapshot_complete(vendor_code_dir: Path) -> bool:
    required = [
...
def ensure_qwen_assets(
    *,
    ...
    if vendor_code_dir is not None and export_code_if_missing and not vendor_snapshot_complete(vendor_code_dir):
    ...
    if manifest_path is not None:
        manifest = {
            ...
            "vendor_snapshot_complete": (
                vendor_snapshot_complete(vendor_code_dir) if vendor_code_dir is not None else None
            ),
